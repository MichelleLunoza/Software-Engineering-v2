...
Public Class CBMS_DetailsForm
    ...
    Private Sub TotalFamilyButton_Click(sender As Object, e As EventArgs) Handles TotalFamilyButton.Click
        ...
        With cmd
            ...
            .CommandText = "SELECT DISTINCT pt.HH_Number AS 'HHN', pt.Name AS 'Head of Family',(SELECT COUNT(ID) FROM PopulationTable WHERE HH_Number=pt.HH_Number) AS 'Total Population',(SELECT COUNT(Gender) FROM PopulationTable WHERE Gender = 'Male' and HH_Number=pt.HH_Number) AS 'Total Male',(SELECT COUNT(Gender) FROM PopulationTable WHERE Gender='Female' and HH_Number=pt.HH_Number) AS 'Total Female',(SELECT COUNT(OFW_Category) FROM PopulationTable WHERE OFW_Category='yes' and HH_Number=pt.HH_Number) AS 'Total OFW',(SELECT COUNT(PWD_Category) FROM PopulationTable WHERE PWD_Category='yes' and HH_Number=pt.HH_Number) AS 'Total PWD',(SELECT COUNT(zero_twelve_months_Category) FROM PopulationTable WHERE zero_twelve_months_Category='yes' and HH_Number=pt.HH_Number) AS '0-12 Months',(SELECT COUNT(two_five_yrs_old_Category) FROM PopulationTable WHERE two_five_yrs_old_Category='yes' and HH_Number=pt.HH_Number) AS '2-5 Yrs Old',(SELECT COUNT(six_twelve_yrs_old_Category) FROM PopulationTable WHERE six_twelve_yrs_old_Category='yes' and HH_Number=pt.HH_Number) AS '6-12 Yrs Old',(SELECT COUNT(thirteen_seventeen_Category) FROM PopulationTable WHERE thirteen_seventeen_Category='yes' and HH_Number=pt.HH_Number) AS '13-17 Yrs Old',(SELECT COUNT(senior_citizen_Category) FROM PopulationTable WHERE senior_citizen_Category='yes' and HH_Number=pt.HH_Number) AS 'Senior Citizen' FROM PopulationTable pt WHERE Family_Category='Head'"
        End With
